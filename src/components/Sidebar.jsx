...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`${sidebarBg} fixed left-0 top-16 h-[calc(100vh-64px)] border-r ${border} w-64 hidden md:flex flex-col z-40 transition-colors duration-300`}
    >
      <div className="flex flex-col h-full p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <item.icon
                      className={`w-5 h-5 transition-colors ${
                        isActive
                          ? "text-indigo-600"
                          : `${textMuted} group-hover:text-indigo-500`
                      }`}
                    />
                    <span
                      className={`text-sm font-semibold tracking-wide ${isActive ? "text-indigo-600" : textPrimary}`}
                    >
                      {item.name}
                    </span>
                  </div>
        ...
        <div className={`mt-auto pt-4 border-t ${border}`}>
          <div className={`${cardBg} p-3 rounded-xl border ${border}`}>
            <NavLink to="/auth">
              <button
                className={`w-full flex items-center gap-3 px-3 py-2.5 ${textMuted} hover:text-rose-500 hover:bg-rose-500/5 rounded-lg transition-all duration-200 group`}
              >
